...
public class Line {
    ...
    */
    ...
    private boolean doLinesIntersect(Line other, Point intersectPoint) {
        ...
        if (!intersectPoint.pointBetween(start, end)) {
            return false;
        }

        /* check for the other line */
        if (!intersectPoint.pointBetween(other.start, other.end)) {
            return false;
        }
        return true;
    }
    ...
    private boolean doParallesIntersect(Line other) {
         ...
         */
        if (start.pointBetween(other.start, other.end)) {
            return true;
        }
        if (end.pointBetween(other.start, other.end)) {
            return true;
        }
        return false;
    }
    ...
    */
    ...
    /**
     * returns the closest intersection point of the rectangle with
     * the start of the line.
     * @param rect the rectangle.
     * @return the closest intersection point if exsits null otherwise.
     */
    public Point closestIntersectionToStartOfLine(Rectangle rect) {
        List<Point> intersctions = rect.intersectionPoints(this);

        return start.closestPointFromPointsArray(intersctions);
    }

    /**
     * returns the collision info of the closest collision from a given
     * intersection points and their coresponds collidable.
     * @param intersections closest intersection point from each collidable.
     * @param collidables collidables intersectng with this line.
     * @return collision object describing the collision.
     */
    public CollisionInfo closestCollisionInfo(List<Point> intersections,
        ...
        /* get the closest point from the intersection closests points */
        closestPoint = start.closestPointFromPointsArray(intersections);

        /* get the index of the closest point related also to the intersections
         * array.
         */
        closestIndex =  intersections.indexOf(closestPoint);

        /* get the collidable object by its index */
        closestCollidable = collidables.get(closestIndex);

        /* return a new collisionInfo inctance */
        return new CollisionInfo(closestCollidable, closestPoint);
    }

    /**
     * method to check if a point lies on thie line.
     * @param p the point.
     * @return true if it is and false otherwise.
     */
    public boolean pointOnLine(Point p) {
        return p.pointBetween(start, end);
    }
